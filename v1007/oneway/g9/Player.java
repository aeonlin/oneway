...
public class Player extends oneway.sim.Player
{
			...
			boolean[] rlights)
	{
		...
		for(int i = 0 ; i <= nsegments ; i++) {
			parkingLotIndex[i] = new Parked(i,i);
			if(i != 0 && i != nsegments) {
				parkingLotIndex[i].amount = left[i].size()+right[i].size();
			}
			if(i == 0) {parkingLotIndex[i].amount = right[i].size();}
			if(i == nsegments) {parkingLotIndex[i].amount = left[i].size();}
		}
		...
		for(int i=0; i<=nsegments; i++) {
			...
			if( index != nsegments && index != 0) {
				if(left[index].size() >= right[index].size()) {
					...
					if(willCrash(movingCars, llights, rlights) 
